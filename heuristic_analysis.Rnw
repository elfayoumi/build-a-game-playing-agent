...
I ran the agent for three different heuristic functions for 200 matches against each oponent, which count to 800 games, which took long time to process. The baseline provided (iterative deeping improved \verb!ID_IMPROVED!) gave the following results in ubuntu linux, 16.04 operating system:
\begin{verbatim}
*************************
...
ID_Improved         83.20%
\end{verbatim}
Three different heuristic functions were used with the gaming agent, while the performance was quite comparable, the heuristic functions that tries to take the apponent number of moves into account performed better than iterative deepining improved. 
\begin{description}
\item{Heuristic function 1}\\
For the first heuristic function, I took the number of player available move $n_p$:
\begin{equation}
\text{Score} = n_p \label{eq:1}
\end{equation}
This heuristic function does not take the oponent remaining number of moves into account and gave the following:
\begin{verbatim}
*************************
   Evaluating: Student   
*************************
...
\item{Heuristic function 2}\\
This time, we try to take into  account the remaining moves of the oponent $n_o$ into account:
\begin{equation}
\text{Score} = n_p - n_o\label{eq:2}
\end{equation}
This equation takes into account the oponent number of moves, so with the number of moves available to the player, take the one that gives the oponent the lowest number of moves. The result was better than baseline:
\begin{verbatim}
*************************
   Evaluating: Student   
*************************
...
Student             83.88%
\end{verbatim}
So, taking the number of moves available to the oponent into account, gave rise to the performance.
\item{Heuristic function 3}\\
Encouraged by the previous result, now what if we try to give very high score for having the available moves to the oponent zero?
\begin{equation}
\text{Score} = \frac{n_p^2}{n_o +0.01}\label{eq:3}
\end{equation}
Equation~\eqref{eq:3} still penalise the palyer for the availble moves of the oponent, but if the opononet number of moves is zero, the available moves to the players is multiplied by 100, so making the heuristic evaluation function very high.
\begin{verbatim}
